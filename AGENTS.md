...
| **Runtime** (symlink) | `.Codex/skills/<name>` → `../../.agents/skills/<name>` | All symlinks — never real dirs |
...
1. Create in `.agents/skills/`:
   ...
   ```
2. Auto-link to `.Codex/skills/`:
   ```bash
   cd .agents/skills && ./link-to-Codex.sh <skill-name>
   ```
