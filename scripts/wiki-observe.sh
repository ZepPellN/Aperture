...
# Wiki Observe — daily pipeline to absorb raw/, refresh search, and rebuild semantic layout
...
# Step 1: Run wiki-absorb via Claude Code (processes raw/ -> wiki/)
echo "[wiki-observe] Step 1/4: /wiki-absorb..." | tee -a "$LOG_FILE"
claude -p "/wiki-absorb" --dangerously-skip-permissions 2>&1 | tee -a "$LOG_FILE"

# Step 2: Refresh QMD index after absorb has updated the vault
echo "[wiki-observe] Step 2/4: qmd update..." | tee -a "$LOG_FILE"
qmd update 2>&1 | tee -a "$LOG_FILE"

echo "[wiki-observe] Step 3/4: qmd embed -f..." | tee -a "$LOG_FILE"
